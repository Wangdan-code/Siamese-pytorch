...
class Siamese(object):
    _defaults = {
        ...
        #-----------------------------------------------------#
        "model_path"        : 'logs/last_epoch_weights.pth',#'model_data/Omniglot_vgg.pth'
        #-----------------------------------------------------#
        #   输入图片的大小。
        #-----------------------------------------------------#
        "input_shape"       : [105, 105],
    ...
    def detect_image(self, image_1, image_2):
        ...
        plt.subplot(1, 2, 1) #subplot() 方法在绘图时需要指定位置
        plt.imshow(np.array(image_1)) #plt.imshow()是Matplotlib中的一个函数，用于显示图像。它可以传递一个二维或三维数组作为image参数， 并将图像数据显示为图形，并对图像进行不同的可视化设置。
